...
public class SeguradoraDaoImpl implements SeguradoraDao {
    ...
    @Override
    public Seguradora create(Seguradora seguradora, Connection connection) throws SeguradoraNotSavedException, SQLException {
        final String sql =
                """
                BEGIN 
                    INSERT INTO T_CON_SEGURADORA(NM_SEGURADORA, NR_CNPJ, ID_VEICULO)
    ...
    public List<Seguradora> readAll() {
        ...
        final String sql = "SELECT * FROM T_CON_SEGURADORA";
        try(Connection connection = DatabaseConnectionFactory.create().get()){
            ...
            logger.warning("nenhum registro encontrado de seguradora: " + e.getMessage());
        }
        return all;
    }
    ...
    public Seguradora update(Seguradora seguradora, Connection connection) throws SeguradoraNotFoundException, SQLException {
        final String sql =
                """
                UPDATE T_CON_SEGURADORA 
                    SET NM_SEGURADORA = ?, NR_CNPJ = ?, ID_VEICULO = ?
                WHERE ID_SEGURADORA = ?;
                """;
        PreparedStatement stmt = connection.prepareStatement(sql);
        ...
        stmt.setObject(4, "id_seguradora");
